...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (video) {
        ...
        // Функция для проверки и перезапуска (на случай если loop не сработает)
        video.addEventListener('ended', function() {
            this.currentTime = 0;
            this.play().catch(e => console.log('Автовоспроизведение заблокировано:', e));
        });
        ...
        // Клик по контейнеру = клик по видео
        if (videoContainer) {
        ...
        video.play().catch(error => {
            ...
            hint.innerHTML = 'Нажмите на видео для воспроизведения';
            hint.style.cssText = `
                position: absolute;
                top: 50%;
                left: 50%;
