...
(function() {
    ...
    async function submitFeedbackForm(event) {
        ...
        if (DEBUG) console.log('📤 Начало отправки формы...');
        ...
        if (!validateFormBeforeSubmit(nameInput, phoneInput, emailInput)) {
            if (DEBUG) console.log('❌ Форма не прошла валидацию');
            return;
        }
        ...
        if (DEBUG) console.log('📦 Данные формы:', { name, phone, email, message });
        ...
        try {
            // 1. Отправляем на почту через FormSubmit
            if (DEBUG) console.log('📧 Отправка на FormSubmit...');
            
            const formData = {
                _subject: `📨 Обратная связь от ${name} - ПолиАрт`,
                _template: 'table',
                '👤 Имя': name,
                '📞 Телефон': phone,
                '📧 Email': email,
                '💬 Сообщение': message || 'Не указано',
                '👥 Статус': currentUser ? 'Авторизованный пользователь' : 'Гость',
                '📅 Дата и время': new Date().toLocaleString('ru-RU'),
                '🌐 Страница': window.location.href,
                _cc: 'pridixalava228@gmail.com',
                _replyto: email,
                _captcha: 'false' // Отключаем капчу для тестирования
            };
            
            if (DEBUG) console.log('📤 Отправляемые данные:', formData);
            ...
            body: JSON.stringify({
                access_key: 'YOUR_ACCESS_KEY', // Получите на web3forms.com
                subject: `Обратная связь от ${name}`,
                name: name,
                phone: phone,
                email: email,
                message: message,
                from_name: 'ПолиАрт Контакты'
            })
        });
            
            if (DEBUG) console.log('📥 Ответ сервера:', response.status, response.statusText);
            ...
            if (DEBUG) console.log('📨 Результат отправки:', result);
            
            if (result.success) {
                // Успешная отправка
                if (DEBUG) console.log('✅ Сообщение успешно отправлено');
                
                // Показываем сообщение об успехе
                showSuccessMessage('✅ Сообщение отправлено! Мы ответим вам в ближайшее время.');
                
                // Очищаем форму
                setTimeout(() => {
                    clearForm(nameInput, phoneInput, emailInput, messageInput);
                }, 1500);
                
            } else {
                throw new Error(result.message || 'Неизвестная ошибка FormSubmit');
            }
            ...
            await tryAlternativeEmailSending(name, phone, email, message);
